...
export default function App() {
  ...
  // --- Test Taking State (Now Persistent) ---
  ...
  const [answers, setAnswers] = useState(() => {
    const saved = localStorage.getItem('cbt_currentAnswers');
    return saved ? JSON.parse(saved) : {};
  });
  ...
  useEffect(() => { localStorage.setItem('cbt_currentIndex', currentQIndex.toString()); }, [currentQIndex]);
  useEffect(() => { localStorage.setItem('cbt_currentAnswers', JSON.stringify(answers)); }, [answers]);
  ...
  const startTest = () => {
    ...
    // Clean start logic
    const newQuestions = shuffleArray(INITIAL_QUESTIONS); // Optional: Reshuffle on start
  ...
  // Helper to clear "Current Session" data when going back to Welcome
  const handleExit = () => {
    setView('welcome');
    // We clear these specific session items so the user starts fresh next time
    // But we leave general settings (app name, etc) and previous results intact
    setAnswers({});
    setCurrentQIndex(0);
    setTimeLeft(0);
    setScore(0);
                ...
                <div className="max-w-3xl mx-auto w-full">
                    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 md:p-10 min-h-[50vh]">
                        ...
                        <h2 className="text-lg md:text-2xl font-medium text-gray-800 mb-6 md:mb-8 leading-relaxed break-words">
                            {question ? question.text : "Question data missing"}
                        </h2>

                        <div className="space-y-3">
                            {question && ['optionA', 'optionB', 'optionC', 'optionD'].map((optKey) => (
                                <button
                                key={optKey}
