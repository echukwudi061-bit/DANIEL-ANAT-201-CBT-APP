...
export default function App() {
  ...
  // 1. Questions - Init with Shuffled Questions
  const [questions, setQuestions] = useState(() => {
    const saved = localStorage.getItem('cbt_questions');
    // If saved exists, use it, otherwise use shuffled Initial Questions
  ...
  // 7. GUEST ID (NEW) - Persistent Per User
  ...
  // Note: GuestID is set once on init, doesn't need effect to update
  ...
  const handleSubmit = () => {
    ...
    const newResult = {
      id: Date.now(),
      guestId: guestId, // Store Guest ID with result
      score: finalScore,
      total: totalPossible,
  ...
  // --- STYLE CONSTANT FOR GUEST ID ---
  const guestIdBadgeStyle = "bg-blue-50 text-blue-800 border border-blue-100 rounded-full px-2 py-0.5 text-[10px] font-bold flex items-center justify-center whitespace-nowrap";
        ...
        <header className="bg-white shadow-sm px-3 md:px-6 py-3 flex justify-between items-center z-20 flex-shrink-0 w-full relative">
          <div className="flex items-center gap-2 overflow-hidden mr-2">
            ...
            <div className="flex flex-col overflow-hidden min-w-0">
              ...
              {/* GUEST ID (Top Left, below Title) */}
              <div className={`${guestIdBadgeStyle} self-start mt-1`}>
                {guestId}
              </div>
            </div>
          </div>
      ...
      <div className="min-h-screen bg-gray-50 p-4 font-sans text-gray-900 overflow-y-auto">
        <div className="max-w-3xl mx-auto space-y-6">
          <div className="bg-white p-6 md:p-8 rounded-2xl shadow-xl text-center">
            ...
            {/* GUEST ID (Centered below Header) */}
